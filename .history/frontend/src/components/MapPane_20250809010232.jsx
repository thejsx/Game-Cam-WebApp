...
export default function MapPane({ sites, selectedSites, onSiteClick }) {
  ...
  useEffect(() => {
    ...
    L.Control.Recenter = L.Control.extend({
      onAdd: function() {
        ...
        const a = L.DomUtil.create('a','',div); a.href="#"; a.innerHTML="Recenter"; a.style.width="100px"; a.style.lineHeight="30px";
  ...
  useEffect(() => {
    ...
    Object.keys(sites).forEach(k => {
      const s = sites[k];
      const lat = s.gps[0], lon = s.gps[1], name = s.site;
  ...
  useEffect(() => {
    ...
    Object.keys(m).forEach(name => {
      if (selectedSites.includes(name)) m[name].setStyle({ color: "green", fillColor: "blue" });
