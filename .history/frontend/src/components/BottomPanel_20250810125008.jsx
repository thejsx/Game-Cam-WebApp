  ...
  return (
    <div className="bottom-panel">
      ...
      {Object.entries(categories).map(([key, title]) => (
        <div key={key} className={`filter-list ${key}`}>
          <div className="filter-title">{title}</div>
          <div className="filter-items">
            {allItems[key] && allItems[key].map(entry => (
              <div key={entry} className={`filter-item ${reverseSettings[key] && !reverseSettings[key].includes(entry) ? 'unavailable' : ''}`}>
                <label>
                  <input 
                    type="checkbox" 
