    ...
    <div className="bottom-panel">
      ...
      <div className="date-range">
        ...
        <div className="date-input">
          <label>Start:</label>
          <input value={start} onChange={e => handleChange('start', e.target.value)} type="date" />
        </div>
        <div className="date-input">
          <label>End:</label>
          <input value={end} onChange={e => handleChange('end', e.target.value)} type="date" />
        </div>
      </div>
          ...
          {videos.length > 0 && (
            <>
              ...
              <button className="videos-button" onClick={() => {
                const selected = Object.fromEntries(Object.entries(videos).filter(([key, value]) => checkedVideos.includes(key)));
                if (Object.keys(selected).length > 0) onOpenPlayer(selected);
