...
export default function MapPane({ sites, onSiteClick }) {
  ...
  useEffect(() => {
    ...
    L.Control.Recenter = L.Control.extend({
      onAdd: function() {
        ...
        a.title = "Recenter map to selected sites or all sites if none selected";
        a.onclick = () => { recenterToSelectedSites(map); return false; };
        return div;
      }
    });
  ...
  useEffect(() => {
    ...
    recenterToSelectedSites(mapRef.current);
  }, [sites]);
  ...
  function recenterToSelectedSites(map) {
    const m = markersRef.current;
    const { selectedSettings } = useGlobalStore.getState();
    const currentSelectedSites = selectedSettings.sites || [];
    
    if (currentSelectedSites.length > 0) {
      const picks = currentSelectedSites.map(n => m[n]).filter(Boolean).map(marker => marker.getLatLng());
      if (picks.length > 0) {
        map.fitBounds(picks, { padding: [50, 50], maxZoom: 17 });
      } else {
        map.setView(center, zoom);
      }
    } else {
      // If no sites selected, fit all sites
      const allMarkers = Object.values(m).map(marker => marker.getLatLng());
      if (allMarkers.length > 0) {
        map.fitBounds(allMarkers, { padding: [50, 50], maxZoom: 17 });
      } else {
        map.setView(center, zoom);
      }
    }
