...
def get_labels(request: Request):
    print("Received request for labels.", request.query_params)
    ...
    if len(qp) == 0:
        ...
        return {
            "sites": data['sites'],
            "animals": ['none'] + data['sorted_videos']['animal_videos'],
            "actions": ['none'] + data['sorted_videos']['actions'],
            "add_labels": ['none'] + data['sorted_videos']['additional_labels'],
    ...
    print("Filters provided:", qp)
    
    # Get filter parameters - empty lists mean no selection for that category
    sites = qp.getlist("sites")
    animals = qp.getlist("animals")
    actions = qp.getlist("actions")
    add_labels = qp.getlist("add_labels")
    start = qp.get("start")
    end = qp.get("end")
