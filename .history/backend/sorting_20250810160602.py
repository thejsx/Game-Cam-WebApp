...
def resort_all_data(data, sites, animals, actions, add_labels, date_labels, restricted):
    ...
    for video, labels in video_labels.items():
        ...
        if restricted and labels.get("restricted", False):
            print(f"Video {video} is restricted.")
            continue

        # Sites check - empty filter means no videos should match
        if labels.get("site") not in sites:
            print("site value:", repr(labels.get("site")))
            print("sites param:", [repr(s) for s in sites], "len:", len(sites))
            break
        ...
        for a in animals:
            ...
            if a == 'none' and len(video_animals) == 0:
                ok_animals = True; break
        if not ok_animals:
            print(f"Video {video} does not match selected animals: {video_animals}")
            continue
        ...
        for act in actions:
            ...
            if act == 'none' and len(labels.get("actions", [])) == 0:
                ok_actions = True; break
        if not ok_actions:
            print(f"Video {video} does not match selected actions: {labels.get('actions', [])}")
            continue
        ...
        for lbl in add_labels:
            ...
            if lbl == 'none' and len(labels.get("additional_labels", [])) == 0:
                ok_add = True; break
        if not ok_add:
            print(f"Video {video} does not match selected additional labels: {labels.get('additional_labels', [])}")
            continue
        
        vdt = parse_dt(labels["time"])
        if not (start_date <= vdt <= end_date):
            print(f"Video {video} is outside date range: {vdt} not in [{start_date}, {end_date}]")
            continue
        videos.append(video)
    return videos
