...
IGNORE_DIRS = {
    ...
    # common user/system junk you saw in the dump:
    ".cache", ".local", ".config", ".nvm", ".npm", "snap"
}
IGNORE_EXTS = {
...
def find_repo_root(start_path=None):
    ...
    while True:
        ...
        if parent == path:
            # Optional guard so we don't walk ~ by mistake:
            raise RuntimeError(f"No .git found above: {orig}. Pass the repo path explicitly.")
        path = parent
...
def copy_to_clipboard(text: str):
    if os.name == "nt":  # Windows
        ...
        print("Copied to clipboard via Windows 'clip' (UTF-8).")
        return
